...
exports.getAppointments = async (req, res) => {
    try {
        const doctorId = req.user?.id || req.query.doctor_id || 1; // Fallback for local testing/dev
...
exports.completeConsultation = async (req, res) => {
    ...
    try {
        ...
        const doctorId = req.user?.id || req.body.doctor_id || 1;
        const { appointmentId, patientId, diagnosis, postRemarks, vitals, prescriptionItems } = req.body;
        ...
        await connection.query(`
            UPDATE appointments 
            SET status = 'Completed', post_remarks = ? 
            WHERE id = ?
        `, [postRemarks || '', appointmentId]);
...
exports.sendReminder = async (req, res) => {
    try {
        const { patientId, reminderText } = req.body;
        
        if (!patientId || !reminderText) {
            return res.status(400).json({ success: false, message: "Missing required fields: patientId, reminderText" });
        }

        // Insert notification into database
        await db.query(
            `INSERT INTO notifications (patient_id, message, status) VALUES (?, ?, 'Unread')`,
            [patientId, reminderText]
        );

        console.log("Push notification created in DB for patient ID: %s", patientId);

        res.json({
            success: true,
            message: "Push notification successfully queued and saved to the database!"
        });
    } catch (error) {
        console.error("Error creating push notification:", error);
        res.status(500).json({ success: false, message: "Failed to create push notification", error: error.message });
    }
};
